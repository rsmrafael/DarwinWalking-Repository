...
AbstractBehavior::AbstractBehavior(Game *game, MitecomCommunicator *communicator)
{
	...
	mWalkingSpeedAvg = config->get<int>("Robot", "walkSpeedAvg", 20);
	mKickingDistanceMax = config->get<int>("Robot", "kickDistMax", 300);
	mMinWalkingTurnAngle = config->get<int>("Robot", "minWalkingTurnAngle", 3);
	mMinTurnAngle = config->get<int>("Robot", "minTurnAngle", 10);
	mYTurnAngle = config->get<int>("Robot", "YTurnAngle", 0);
	mLeftTurnSpeed = config->get<int>("Robot", "LeftTurnSpeed", 1);
	mRightTurnSpeed = config->get<int>("Robot", "RightTurnSpeed", 1);
	...
	mHaveGoalie = false;
	mIsPenalized = false;
	mUseMitecom = config->get<bool>("AI", "useMitecom", true);
	mDynamicRoleChange = config->get<bool>("AI", "changeRoleDynamically", true);
...
{
	if( state == IGame::SET) {
		changeRole( Config::getInstance()->getString("AI", "defaultAI", "striker"));
	}
}
...
void AbstractBehavior::updateCommunicator()
{
	...
	if( getBallStatus().getObjectStatus() == VisionStatus::OBJECT_FOUND_STABLE) {
		double ballX = getBallVector().getX();
		double ballY = getBallVector().getY();
...
void AbstractBehavior::changeRole( int role)
{
	...
	case ROLE_DEFENDER:
	case ROLE_GOALIE:
	case ROLE_FIELD_RUNNER:
	case ROLE_PLACE_ROBOT:
		mCurrentRole = role;
		break;
	default:
...
void AbstractBehavior::changeRole(string role)
{
	...
	if( role == "goalie") {
		...
		mCurrentRole = ROLE_FIELD_RUNNER;
	} else if( role == "placerobot") {
		mCurrentRole = ROLE_PLACE_ROBOT;
	} else {
