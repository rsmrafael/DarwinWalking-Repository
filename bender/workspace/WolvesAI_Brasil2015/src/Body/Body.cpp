...
void Body::setParalysis(bool paralyze) {
	if (paralyze && !mParalysis) {
		Stop();
	}
	mParalysis = paralyze;
}
...
double Body::GetBatteryVoltage() {
	const ICommunicator::statusMessage_t* status = mComm->GetStatusMessage();
	if (status != NULL) {
...
const ICommunicator::statusMessage_t* Body::GetStatusMessage() {
