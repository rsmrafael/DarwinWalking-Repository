...
void IncreaseValue(unsigned char large) {
	...
	switch(row) {
		...
		case ROLL_OFFSET_ROW:
			if(large == 1)
				Walker_Params.R_OFFSET += 1.0f;
			else
				Walker_Params.R_OFFSET += 0.1f;
			dbgu_printf("%.1f    ", Walker_Params.R_OFFSET);
			break;

		case PITCH_OFFSET_ROW:
			if(large == 1)
				Walker_Params.P_OFFSET += 1.0f;
			else
				Walker_Params.P_OFFSET += 0.1f;
			dbgu_printf("%.1f    ", Walker_Params.P_OFFSET);
			break;

		case YAW_OFFSET_ROW:
			if(large == 1)
				Walker_Params.Y_OFFSET += 1.0f;
			else
				Walker_Params.Y_OFFSET += 0.1f;
			dbgu_printf("%.1f    ", Walker_Params.Y_OFFSET);
			break;

		case HIP_PITCH_OFFSET_ROW:
			if(large == 1)
				Walker_Params.HIP_PITCH_OFFSET += 1.0f;
			else
				Walker_Params.HIP_PITCH_OFFSET += 0.1f;
		...
		case DSP_RATIO_ROW:
			if(large == 1)
				Walker_Params.DSP_RATIO += 0.1f;
			else
				Walker_Params.DSP_RATIO += 0.01f;
			dbgu_printf("%.2f    ", Walker_Params.DSP_RATIO);
			break;

		case STEP_FORWARDBACK_RATIO_ROW:
			if(large == 1)
				Walker_Params.STEP_FB_RATIO += 0.1f;
			else
				Walker_Params.STEP_FB_RATIO += 0.01f;
		...
		case SWING_RIGHTLEFT_ROW:
			if(large == 1)
				Walker_Params.Y_SWAP_AMPLITUDE += 0.10f;
			else
				Walker_Params.Y_SWAP_AMPLITUDE += 0.1f;
		...
		case PELVIS_OFFSET_ROW:
			if(large == 1)
				Walker_Params.PELVIS_OFFSET += 1.0f;
			else
				Walker_Params.PELVIS_OFFSET += 0.1f;
			dbgu_printf("%.1f    ", Walker_Params.PELVIS_OFFSET);
			break;

		case ARM_SWING_GAIN_ROW:
			if(large == 1)
				Walker_Params.ARM_SWING_GAIN += 0.10f;
			else
				Walker_Params.ARM_SWING_GAIN += 0.1f;
			dbgu_printf("%.1f    ", Walker_Params.ARM_SWING_GAIN);
			break;

		case BAL_KNEE_GAIN_ROW:
			if(large == 1)
				Walker_Params.BALANCE_KNEE_GAIN += 0.1f;
			else
				Walker_Params.BALANCE_KNEE_GAIN += 0.01f;
			dbgu_printf("%.2f    ", Walker_Params.BALANCE_KNEE_GAIN);
			break;

		case BAL_ANKLE_PITCH_GAIN_ROW:
			if(large == 1)
				Walker_Params.BALANCE_ANKLE_PITCH_GAIN += 0.1f;
			else
				Walker_Params.BALANCE_ANKLE_PITCH_GAIN += 0.01f;
			dbgu_printf("%.2f    ", Walker_Params.BALANCE_ANKLE_PITCH_GAIN);
			break;

		case BAL_HIP_ROLL_GAIN_ROW:
			if(large == 1)
				Walker_Params.BALANCE_HIP_ROLL_GAIN += 0.1f;
			else
				Walker_Params.BALANCE_HIP_ROLL_GAIN += 0.01f;
			dbgu_printf("%.2f    ", Walker_Params.BALANCE_HIP_ROLL_GAIN);
			break;

		case BAL_ANKLE_ROLL_GAIN_ROW:
			if(large == 1)
				Walker_Params.BALANCE_ANKLE_ROLL_GAIN += 0.1f;
			else
				Walker_Params.BALANCE_ANKLE_ROLL_GAIN += 0.01f;
...
void DecreaseValue(unsigned char large) {
	...
	case SWING_RIGHTLEFT_ROW:
		if(large == 1)
			Walker_Params.Y_SWAP_AMPLITUDE -= 1.0f;
		else
			Walker_Params.Y_SWAP_AMPLITUDE -= 0.1f;
		dbgu_printf("%.1f    ", Walker_Params.Y_SWAP_AMPLITUDE);
		break;
	...
	case ARM_SWING_GAIN_ROW:
		if(large == 1)
			Walker_Params.ARM_SWING_GAIN -= 1.0f;
		else
			Walker_Params.ARM_SWING_GAIN -= 0.1f;
		dbgu_printf("%.1f    ", Walker_Params.ARM_SWING_GAIN);
		break;
