...
bool GoalieDetection::execute() {
	...
	if (goalPoles < 2) {
		...
		cleanUpList(cyanClouds);
		cleanUpList(magentaClouds);
		return true;
	}
...
void GoalieDetection::configChanged() {
	...
	mDebugDrawings = config->get<int>("Vision", "Goalie_debugDrawings", 0);
	mGoalieDetectionEnabled = config->get<int>("Vision", "Goalie_DetectionEnabled", 1);
	mMinColorCloudPoints = config->get<int>("Vision", "Robot_minColorCloudPoints", 6);
	mMinCloudDistance = config->get<int>("Vision", "minCloudDistance", 10);
	mSteps = config->get<int>("Vision", "Robot_IterateSteps", 1);
	mMinRobotPointsPercentage = config->get<double>("Vision", "Goalie_RobotPointsPercentage", 0.6);
	mRobotFeetWidthPercentage = config->get<double>("Vision", "Goalie_FeetPercentageofBody", 0.2);
	mMaxY_Black = config->get<int>("Vision", "Black_MaxY", 80);
	mMaxV_Gray =  config->get<int>("Vision", "Gray_MaxV", 60);
	mMinU_Cyan = config->get<int>("Vision", "Cyan_minU", 160);
	mMaxV_Cyan = config->get<int>("Vision", "Cyan_maxV", 110);
	mMinU_Magenta = config->get<int>("Vision", "Magenta_minU", 140);
	mMinV_Magenta = config->get<int>("Vision", "Magenta_minV", 140);
}
