...
double Point::calcluateDistanceTo(const Point &point) const{
	double dist = sqrt((mX-point.mX)*(mX-point.mX) + (mY-point.mY)*(mY-point.mY));
//	Debugger::DEBUG("Point", "p1 %i/%i , p2: %i/%i  - dist: %f", mX, mY, point.getX(), point.getY(), dist);
	return dist;
}
