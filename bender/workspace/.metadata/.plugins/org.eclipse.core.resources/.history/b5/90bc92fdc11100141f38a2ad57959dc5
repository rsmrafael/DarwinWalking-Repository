...
void BehaviorStriker::configChanged()
{
	...
	mSearchNextPoleTimeout	= Config::GetValue<uint64_t>("AI", "searchNextPoleTimeout", 5000L);
	mPositionBallDistanceMin = Config::GetInt("AI", "position_ballDistanceMin", 100);
	mPositionBallDistanceMax = Config::GetInt("AI", "position_ballDistanceMax", 200);
	mKickTargetDistanceMax = Config::GetInt("AI", "kick_targetDistanceMax", 1800, "maximal possible distance of kick target");
...
#endif
	if( (int)getBall().lastVector.getLength() < mMoveToBallDistance) {
		//getBodyAction().stop();
		if (!mSearchGoal) {
...
{
	if( getPolesRelative().getObjects().size() >= 2
...
void BehaviorStriker::seekTarget()
{
	...
	*/
	...
	vector<Object> goalPoles = getPolesRelative().getDistanceSortedObjects();
	if( goalPoles.size() >= 2) {
		Vector vec1, vec2;
		vector<Object>::const_iterator it = goalPoles.begin();
		vec1 = it->lastVector;
		++it;
		vec2 = it->lastVector;
