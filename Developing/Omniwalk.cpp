...
void Omniwalk::getHaltPosition(int leg, double *FootPose){
	double eta,legRoll,legPitch,legYaw,footRoll,footPitch;
	
	legYaw = 0;
	MotionPattern::getInstance()->getHaltPosition(leg, m_Tau,
        m_Vx, m_Vy, m_Vphi, *eta, *legRoll, *legPitch, *legYaw,
        *footRoll, *footPitch);
	m_LegInterface(FootPose,eta,legRoll,legPitch,legYaw);
	FootPose[3] = footRoll;
	FootPose[4] = footPitch;
	FootPose[5] = legYaw;
}
