...
:	mMaxFieldlineError(10),
	...
	mKernelSizeContur(5),
	mMinWhiteCloud(1500),
	mScanlineDistanceColumn(3),
	mScanlineDistanceRow(3),
	mContoursWhite(),
	mContoursGreen()
...
void BallDetectionOpenCV::cutContour() {
	//	printf("\n\n\n-----Number of White Points in picture: %d-----\n\n\n", whiteCounter);
//	if (whiteCounter > mMinWhiteCloud) {

	//TODO dilate + erode
	cv::Mat zer = cv::Mat::zeros(mCamHeight, mCamWidth, CV_8UC3);
			zer.copyTo(mContoursWhite);
			cv::Mat zer2 = cv::Mat::zeros(mCamHeight, mCamWidth, CV_8UC3);
			cv::Mat mEasyContursGreen;
			zer2.copyTo(mContoursGreen);



		cv::Mat onlyOneWhite = cv::Mat(mCamHeight, mCamWidth, CV_8UC3);
		cv::Mat onlyOneGreen = cv::Mat(mCamHeight, mCamWidth, CV_8UC3);

		//		mContoursWhite.zeros(
//				mContoursWhite.cols,
//				mContoursWhite.rows,
//				mContoursWhite.type());
//		mContoursGreen.zeros(
//				mContoursGreen.cols,
//				mContoursGreen.rows,
//				mContoursGreen.type());

		mContoursWhite = getContourWhite().getMat();
		mContoursGreen = getContourGreen().getMat();

		cv::Vec3b greenColor = cv::Vec3b(255, 0, 0);
		cv::Vec3b whiteColor = cv::Vec3b(255, 255, 90);

		//green
		cv::inRange(mContoursGreen, greenColor, greenColor, onlyOneGreen);
		int dilation_type;
		dilation_type = cv::MORPH_RECT;

		cv::Mat element = cv::getStructuringElement( dilation_type,
				cv::Size( 2*mKernelSizeContur + 1, 2*mKernelSizeContur + 1),
				cv::Point( mKernelSizeContur, mKernelSizeContur) );
		cv::dilate(onlyOneGreen, onlyOneGreen, element);
		cv::erode(onlyOneGreen, onlyOneGreen, element);

		//white
		dilation_type = cv::MORPH_ELLIPSE;
		cv::inRange(mContoursWhite, whiteColor, whiteColor, onlyOneWhite);
		element = cv::getStructuringElement( dilation_type,
				cv::Size( 4*mKernelSizeContur + 1, 4*mKernelSizeContur + 1),
				cv::Point( mKernelSizeContur, mKernelSizeContur) );
		cv::dilate(onlyOneWhite, onlyOneWhite, element);
		cv::erode(onlyOneWhite, onlyOneWhite, element);

		for (int i = 0; i < mContoursWhite.cols; i = i + mScanlineDistanceColumn) {
			for (int j = 0; j < mContoursWhite.rows; j = j + mScanlineDistanceRow) {
		//	for (int i = 0; i < mEasyContursWhite.cols; i++) {
		//		for (int j = 0; j < mEasyContursWhite.rows; j++) {
				if (onlyOneWhite.at<uchar>(cv::Point(i,j)) &&
						!onlyOneGreen.at<uchar>(cv::Point(i,j))){
					mContoursWhite.at<cv::Vec3b>(j, i) = whiteColor;
//				} else {
//					mContoursWhite.at<cv::Vec3b>(j, i) = cv::Vec3b(0, 0, 0);
				}
			}
		}
//	}
		//TODO asd
//	getEasyContour().setMat(mContoursWhite);
}
...
//	mSrc = getOpenCVImage().getOpenCVYUV();
	...
	//-----------start BallDetection Tom
//	cutContour();
	mSrc = getContourWhite().getMat();
	cv::cvtColor(mSrc, mSrcGray, CV_BGR2GRAY);
	//-----------end BallDetection Tom
...
bool BallDetectionOpenCV::checkGoalPoles(const CircleModel &currentBall) {
	Object first = getGoalPoles().getObjects().front();
	Object second = getGoalPoles().getObjects().back();

	Object currentBallBox = Object(
			currentBall.getCenter().getX() - currentBall.getRadius(),
			currentBall.getCenter().getY() - currentBall.getRadius(),
			currentBall.getRadius(),
			currentBall.getRadius());
	if (first.doBoundingBoxesOverlap(currentBallBox) ||
			second.doBoundingBoxesOverlap(currentBallBox)) {
		return true;
	}
	return false;
}

bool BallDetectionOpenCV::checkConturMinY(CircleModel current) const {
	int ballX = current.getCenter().getX();
	int ballMinY = current.getCenter().getY() - current.getRadius();
	if (getFieldLines().getY(ballX) > ballMinY) {
		return true;
	}
	return false;
}
...
bool BallDetectionOpenCV::isColorMatching(CircleModel *circle) {
	...
	int greenCnt  = 0;
	int orangeCnt = 0;
	//const YUVImage* yuvImage = &getCuttetImage();
...
bool BallDetectionOpenCV::filterFieldLines(const CircleModel &ball) const
{
	const Point &ballCenter = ball.getCenter();
	//if (mIsFilterFieldLine) {
	//getBodyStatus().getTilt() > 25 &&
		if(checkConturMinY(ball) &&
				!getFieldLines().isPointInField(ballCenter, mMaxFieldlineError)) {
