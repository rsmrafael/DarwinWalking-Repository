...
MitecomCommunicator::MitecomCommunicator()
{
	...
	mTeamID = Config::getInstance()->get<uint16_t>("Game", "teamID", 23);
	mInfo.robotID = 0;
	uint16_t port = Config::getInstance()->get<uint16_t>("Mitecom", "CommunicationPort", 4444);
...
void MitecomCommunicator::sendUpdate()
{
	...
	if( messageDataPtr != NULL) {
		if( mNetwork != NULL) {
			...
			Debugger::INFO("Mitecom", "Network crashed!");
		}
		free(messageDataPtr);
	}
}
