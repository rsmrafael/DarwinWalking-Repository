...
PointCloudObject_t RobotDetection::getNearestCloudToPoint(const Point &point, vector<PointCloudObject_t> &cloudList) const
{
	...
	for( cloudIt = cloudList.begin(); cloudIt != cloudList.end(); ++cloudIt) {
		...
		dist = point.calcluateDistanceTo(center);
...
void RobotDetection::configChanged()
{
	...
	//TODO implement Debug Config -> error
	...
	mMinCloudPoints 	 = Config::GetValue<size_t>("Vision", "Robot_minCloudPoints", (size_t)12);
	mMinColorCloudPoints = Config::GetValue<size_t>("Vision", "Robot_minColorCloudPoints", (size_t)6);
