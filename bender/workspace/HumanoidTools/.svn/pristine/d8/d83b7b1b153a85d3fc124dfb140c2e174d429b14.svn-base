...
public class ImageVisualizer extends JScrollPane implements MouseListener,
	...
	private YUVImageRepresentation mImageRep;
	...
	private ActionListener mMenuListener = new ActionListener() {
		public void actionPerformed(ActionEvent event) {
			...
			filterChannels(event.getActionCommand());
		}
	};
	...
	public ImageVisualizer(YUVImageRepresentation image, ColorInfo cf) {
		super();
		mImageRep = image;
		setSize(image.getWidth(), image.getHeigth());
		...
		mScale = 1.0;
		mOriginalImage = image.getBufferedImage();
		...
		JMenuItem item = new JMenuItem("YUV");
		item.addActionListener(mMenuListener);
		mMenu.add(item);

		JMenuItem itemY = new JMenuItem("Y");
		itemY.addActionListener(mMenuListener);
		mMenu.add(itemY);

		JMenuItem itemU = new JMenuItem("U");
		itemU.addActionListener(mMenuListener);
		mMenu.add(itemU);

		JMenuItem itemV = new JMenuItem("V");
		itemV.addActionListener(mMenuListener);
		mMenu.add(itemV);
	...
	private void filterChannels(String filter) {

		if (filter.equals("YUV")) {
			setImage(mImageRep.getBufferedImage());
		} else if (filter.equals("Y")) {
			setImage(mImageRep.getYChannel());
		} else if (filter.equals("U")) {
			setImage(mImageRep.getUChannel());
		} else if (filter.equals("V")) {
			setImage(mImageRep.getVChannel());
		}
		...
		if (mDrawObjects.size() > 0) {
			...
			synchronized (mDrawObjects) {
				for (DrawObject object : mDrawObjects) {
					if (object != null) {
						if (mSelectedAnnotations == null
								|| mSelectedAnnotations.contains(","
										+ object.getSource().toLowerCase()
										+ ",")) {
	...
	public void mouseMoved(MouseEvent e) {
		...
		if (mImageRep != null) {
			...
			int maxWidth = mImageRep.getWidth();
			if (getWidth() > maxWidth)
				maxWidth = getImage().getWidth();
			int maxHeight = mImageRep.getHeigth();
			if (getHeight() > maxHeight)
				maxHeight = getImage().getHeight();

			if (x >= 0 && y >= 0 && x < maxWidth && y < maxHeight) {
				mColorInfo.setColor(mImageRep.getColor(e.getX(), e.getY()));
			}
		}
