...
//	Debugger::DEBUG("ModuleManager", "Executing modules...");
	...
	do {
		...
		if (executableModule == NULL && mThreadPool->GetActiveJobs() == 0) {
			...
			pthread_mutex_unlock(&mMutex);
			Debugger::ERR("ModuleManager",
					"Dependency problem: No non-executed modules have all required representations available! Unexecuted Modules %s",
					unexed.c_str());
			break;
		}
		do {
...
#ifdef JENKINS
	mCalculateExecutionTimeMetrics = Config::GetBoolean("ModuleManager", "calculateExecutionTimeMetrics", true);
#else
	mCalculateExecutionTimeMetrics = Config::GetBoolean("ModuleManager", "calculateExecutionTimeMetrics", false);
#endif
}
